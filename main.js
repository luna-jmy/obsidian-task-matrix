...
function notifyEditResult(result, task, message) {
  if (result === "missing") {
    new import_obsidian2.Notice(`Line ${task.lineNumber} not found in ${task.filePath}`);
    return;
  }
  if (result === "updated") new import_obsidian2.Notice(message);
}
...
var TaskMatrixPlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
  ...
  async refreshTasks(showNotice = false) {
    ...
    if (showNotice) {
      new import_obsidian2.Notice(`Task matrix refreshed: ${this.tasks.length} tasks`);
    }
  }
  ...
  handleVaultChange(file) {
    if (file instanceof import_obsidian2.TFile && this.shouldTrackMarkdownPath(file.path)) {
      this.scheduleRefresh();
    }
  }
  handleVaultRename(file, oldPath) {
    const currentPathMatches = file instanceof import_obsidian2.TFile && this.shouldTrackMarkdownPath(file.path);
  ...
  async cancelTask(task) {
    ...
    const result = await editTaskLine(
      this.app,
      task,
      (line) => setCheckboxMarker(line, cancelledMarker)
    );
        ...
        (result) => {
          void (async () => {
            if (result.adjustDueDate) {
              updates.dueDate = today;
              new import_obsidian2.Notice("Due date adjusted to today");
            } else if (result.addConflictTag) {
              new import_obsidian2.Notice("Date conflict tag added");
            } else {
              return;
            }
  ...
  async applyGtdStateChanges(task, newState, updates, tagToAdd, removeTags, shouldComplete, addConflictTag) {
    ...
    const result = await editTaskLine(this.app, task, (line) => {
      ...
      if (updates.startDate !== void 0) {
        body = setDateField(body, "\u{1F6EB}", updates.startDate);
      }
      if (updates.dueDate !== void 0) {
        body = setDateField(body, "\u{1F4C5}", updates.dueDate);
      }
      return compactLine(`${indent}${body}`);
    });
    if (result === "updated") new import_obsidian2.Notice(`Moved to ${newState}`);
  }
  async moveTaskToQuadrant(task, newQuadrant) {
    ...
    if (result === "updated") new import_obsidian2.Notice(`Moved to ${newQuadrant}`);
  }
};
var TaskMatrixView = class extends import_obsidian2.ItemView {
  constructor(leaf, plugin) {
    ...
    this.collapsedGtdColumns = /* @__PURE__ */ new Set();
  ...
  async renderBodyContent(parent) {
    ...
    if (tasks.length === 0) {
      ...
      empty.createEl("p", {
        text: this.searchQuery || this.getActiveDateFilterCount() > 0 ? "The current search or date filters did not match any tasks." : scanFolders.length > 0 ? `No tasks found in the scanned folders: ${scanFolders.join(", ")}. Adjust the scan folders setting to cover more of your vault.` : "Create markdown tasks in your vault, then refresh this view."
      });
      return;
    }
    if (this.currentView === "list") {
  ...
  collectCalendarItems(tasks) {
    ...
    for (const task of tasks) {
      ...
      if (showInProcessTasks && task.displayStatus !== "cancelled" && this.isValidIso(task.startDate) && this.isValidIso(task.dueDate) && task.startDate < task.dueDate) {
        ...
        for (let day = new Date(start.getFullYear(), start.getMonth(), start.getDate() + 1); this.toCalendarIso(day) < task.dueDate; day.setDate(day.getDate() + 1)) {
  ...
  async openTask(task) {
    const file = this.app.vault.getAbstractFileByPath(task.filePath);
    if (!(file instanceof import_obsidian2.TFile)) {
      new import_obsidian2.Notice(`File not found: ${task.filePath}`);
      return;
    }
    const leaf = this.app.workspace.getLeaf(true);
    await leaf.openFile(file);
    new import_obsidian2.Notice(`Opened ${task.filePath}:${task.lineNumber}`);
  }
};
var TaskEditModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    const descInput = new import_obsidian2.TextComponent(descRow);
    ...
    const prioritySelect = new import_obsidian2.DropdownComponent(priorityRow);
    ...
    const idInput = new import_obsidian2.TextComponent(idInputRow);
    idInput.setValue(taskId);
    idInput.inputEl.addClass("task-matrix-grow-input");
    const generateIdBtn = new import_obsidian2.ButtonComponent(idInputRow).setButtonText("\u{1F3B2}").setTooltip("Generate random ID").onClick(() => {
    ...
    const dependsSelect = new import_obsidian2.DropdownComponent(dependsRow);
    ...
    new import_obsidian2.ButtonComponent(buttons).setButtonText(this.isCreateMode ? "Create" : "Save").setCta().onClick(() => {
      void (async () => {
        const updates = {
  ...
  async createTask(updates, addConflictTag = false) {
    ...
    if (!desc) {
      new import_obsidian2.Notice("Task description is required");
      return;
    }
    let targetFile;
    ...
    if (newTaskTargetPath) {
      ...
      if (!targetFile) {
        new import_obsidian2.Notice(`Target note not found: ${resolvedPath}`);
        return;
      }
    } else {
      ...
      if (activeFile && activeFile.extension === "md") {
        ...
        new import_obsidian2.Notice("Set the target note path or open a Markdown file.");
        return;
      }
    }
    let taskLine = `- [ ] ${desc}`;
    ...
    if (newTaskTargetHeading) {
      ...
      if (!inserted && insertionError) {
        new import_obsidian2.Notice(`Cannot add task: ${insertionError}`);
        return;
      }
    } else {
      await editFileContent(this.app, targetFile, (content) => appendLine(content, taskLine));
    }
    new import_obsidian2.Notice(`Task added to ${targetFile.path}`);
  }
  ...
  async saveTask(updates, addConflictTag = false) {
    ...
    const result = await editTaskLine(this.app, task, (line) => {
      ...
      if (description !== void 0 && description !== task.description) {
...
var DateConflictModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.ButtonComponent(buttonRow).setButtonText("No, add conflict tag").onClick(() => {
      this.result = { adjustDueDate: false, addConflictTag: true };
      this.onConfirm(this.result);
      this.close();
    });
    new import_obsidian2.ButtonComponent(buttonRow).setButtonText("Yes, adjust due date").setCta().onClick(() => {
...
var DeleteTaskModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.ButtonComponent(buttonRow).setButtonText("Delete").setWarning().onClick(() => {
      this.onConfirm();
      this.close();
    });
  }
...
var TaskMatrixSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Scanning and views").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Scan folders").setDesc(
      ...
      (text) => text.setPlaceholder(DEFAULT_SETTINGS.scanFolders.join(", ")).setValue(this.plugin.settings.scanFolders.join(", ")).onChange((value) => {
        this.plugin.settings.scanFolders = value.split(",").map((s) => s.trim()).filter(Boolean);
        this.persistSettings(true);
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Default view").setDesc("Choose which dashboard opens first.").addDropdown(
    ...
    new import_obsidian2.Setting(containerEl).setName("Exclude folders").setDesc("Comma-separated list of folder paths to exclude from task scanning.").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Open location").setDesc("Where to open the task matrix view.").addDropdown(
    ...
    new import_obsidian2.Setting(containerEl).setName("Completion markers").setDesc("Markers for completed tasks, separated by commas.").addText(
      (text) => text.setValue(this.plugin.settings.completionMarkers.join(", ")).onChange((value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Exclude markers").setDesc("Checkbox contents to exclude from task views and statistics, separated by commas.").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Cancelled markers").setDesc("Checkbox contents that indicate a cancelled task (comma-separated). Default: -").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Include completed tasks").setDesc("Show completed tasks in the matrix.").addToggle(
    ...
    if (this.plugin.settings.includeCompleted) {
      new import_obsidian2.Setting(containerEl).setName("Show completed tasks without due date").setDesc("When disabled, completed tasks are shown only if they have a due date.").addToggle(
      ...
      if (!this.plugin.settings.includeCompletedWithoutDueDate) {
        new import_obsidian2.Setting(containerEl).setName("Completed task display range").setDesc("Only show completed tasks finished within this many months. Uses completion date first, then due date. Set to 0 to show all.").addSlider(
    ...
    new import_obsidian2.Setting(containerEl).setName("Track completion date").setDesc("When enabled, automatically add \u2705 yyyy-mm-dd to tasks when they are marked as completed.").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("Urgent days range").setDesc("Number of days to consider a task as urgent (1-7). Default: 1 (today only). 2 = today+tomorrow, 3 = today+2 days, etc.").addSlider(
    ...
    new import_obsidian2.Setting(containerEl).setName("Due date display range").setDesc("Only show tasks due within this many months (overdue tasks are always shown). Set to 0 to show all.").addSlider(
    ...
    new import_obsidian2.Setting(containerEl).setName("Hide future start tasks").setDesc("Hide tasks whose start date is more than 1 month from now.").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("New tasks").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Target note path").setDesc("Path template for new tasks. Date placeholders are replaced with the current date. Leave empty to use the active note.").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("Target heading").setDesc("Insert new tasks under this heading. Leave empty to append at end of file.").addText(
    ...
    new import_obsidian2.Setting(containerEl).setName("List view").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Show cancelled tasks").setDesc("Show cancelled tasks in list view.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.listShowCancelled).onChange((value) => {
        this.plugin.settings.listShowCancelled = value;
        this.persistSettings(true);
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Group by folder").setDesc("Group tasks by their containing folder in list view.").addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.listGroupByFolder).onChange((value) => {
        this.plugin.settings.listGroupByFolder = value;
        this.persistSettings();
      })
    );
    new import_obsidian2.Setting(containerEl).setName("Folder grouping depth").setDesc("How many folder levels to display for grouping (1 = top level only, 2 = two levels, etc.).").addSlider(
    ...
    new import_obsidian2.Setting(containerEl).setName("Calendar view").setHeading();
    new import_obsidian2.Setting(containerEl).setName("First day of week").setDesc("Choose the first day shown in week and month views.").addDropdown(
    ...
    new import_obsidian2.Setting(containerEl).setName("Show weekends in week view").setDesc("Show weekend columns in the week view.").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("Show weekends in month view").setDesc("Show weekend columns in the month view.").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("Show in-progress tasks").setDesc("For tasks with both start and due dates, show them on each day between start and due in calendar views.").addToggle(
    ...
    new import_obsidian2.Setting(containerEl).setName("Show full month in list").setDesc("When enabled, list mode shows every day of the month. When disabled, only shows dates that have tasks.").addToggle(
